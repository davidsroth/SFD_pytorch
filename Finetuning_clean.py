...
train_data = "Toy Dataset/index.csv"
...
def train_model(model, criterion, optimizer, num_classes, num_epochs = 100):
    ...
    for epoch in range(num_epochs):
        ...
        for i,(img,label) in enumerate(train_loader):
            ...
            if use_cuda:
                data, target = Variable(img.cuda(), requires_grad = True), Variable(torch.Tensor(label).cuda())
            else:
                data, target = Variable(img, requires_grad = True), Variable(torch.Tensor(label))
...
myModel.conv3_3_norm_mbox_gender = nn.Conv2d(256, 4, kernel_size=3, stride=1, padding=1)
...
myModel.conv6_gender = nn.Conv2d(512, 2, kernel_size=3, stride=1, padding=1)
conv7_2_gender = nn.Conv2d(256, 2, kernel_size=3, stride=1, padding=1)
